...
export default function Header() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      setIsScrolled(window.scrollY > 150); // Trigger sticky header a bit later for smoother feel
    };
  ...
  return (
    <>
      {/* 
        --- MAIN HEADER (Static at the top) ---
        This header stays in the document flow. 
      */}
            ...
            <div className="flex-1 flex flex-col gap-4">
              ...
              <div className="hidden md:flex items-center justify-between gap-6">
                ...
                <div className="flex items-center space-x-6 flex-shrink-0">
                  ...
                  <button 
                    className="flex items-center gap-3 bg-brand-900 text-white px-5 py-2.5 rounded-full hover:bg-brand-800 transition-all shadow-sm hover:shadow-md active:scale-95"
                    onClick={() => setIsCartOpen(true)}
                  >
                    <div className="relative">
                    ...
                    <span className="text-sm font-bold tracking-wide">${(0.00).toFixed(2)}</span>
                  </button>
                </div>
              </div>
              ...
              <div className="hidden md:flex items-center justify-between border-t border-gray-100 pt-3">
                <nav className="flex items-center space-x-8 text-sm font-bold text-gray-700">
                  ...
                  <div 
                    ...
                    onMouseEnter={() => setActiveMegaMenu('main')}
                  ...
                  >
                    ...
                    <div className={cn(
                      "absolute top-full left-0 w-[800px] pt-4 transition-all duration-200 origin-top-left z-50",
                      activeMegaMenu === 'main' ? "opacity-100 scale-100 pointer-events-auto" : "opacity-0 scale-95 pointer-events-none"
                    )}>
                      <MegaMenuContent />
                    </div>
                  </div>
      ...
      {/* 
        ...
        This header is fixed and animates in smoothly.
      */}
      <div className={cn(
        "fixed top-0 left-0 right-0 z-50 bg-white shadow-md transition-transform duration-500 ease-in-out",
        ...
        <div className="container mx-auto px-4 md:px-6 py-3 flex items-center justify-between gap-4">
          ...
          <div className="hidden md:flex items-center space-x-6">
            <div 
              className="relative group"
              onMouseEnter={() => setActiveMegaMenu('sticky')}
              onMouseLeave={() => setActiveMegaMenu(null)}
            >
              ...
              {/* Invisible Bridge */}
              <div className={cn(
                "absolute top-full left-0 w-[800px] pt-4 transition-all duration-200 origin-top-left",
                activeMegaMenu === 'sticky' ? "opacity-100 scale-100 pointer-events-auto" : "opacity-0 scale-95 pointer-events-none"
              )}>
                <MegaMenuContent />
              </div>
            </div>
            <Link to="/deals" className="text-sm font-bold text-accent-600 hover:text-accent-700">Deals</Link>
            <Link to="/shop" className="text-sm font-bold text-gray-700 hover:text-brand-700">Shop</Link>
            <Link to="/services" className="text-sm font-bold text-gray-700 hover:text-brand-700">Services</Link>
          </div>
          ...
          <div className="flex items-center justify-end space-x-4 md:space-x-6">
            <Link to="/account" className="hidden md:block text-gray-500 hover:text-brand-700 transition-colors">
              <User className="w-6 h-6" strokeWidth={1.5} />
            </Link>
            <button 
              className="flex items-center gap-2 text-gray-900 hover:text-brand-700 transition-colors"
              onClick={() => setIsCartOpen(true)}
            >
              <div className="relative">
                <ShoppingCart className="w-6 h-6" strokeWidth={1.5} />
              ...
              <span className="hidden md:block text-sm font-bold">${(0.00).toFixed(2)}</span>
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[60] bg-gray-900/60 backdrop-blur-sm md:hidden" onClick={() => setIsMobileMenuOpen(false)}>
          ...
          >
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              ...
              <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 text-gray-400 hover:bg-gray-200 rounded-full transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
                  ...
                  {CATEGORIES.map(category => (
                    <li key={category}>
                      <Link 
                        ...
                        className="block text-gray-600 font-medium hover:text-brand-600 text-lg"
                      >
                        {category}
                      </Link>
                    </li>
